...
@RestController
@RequestMapping("/RecetasCocinaCloud")
public class RecetasCocinaController {
	...
	public String helloWorld() {
	 ...
	 public ResponseEntity<?> getRecetas(){
		 List<RecetaCocina> recetas = recetaCocinaService.findAll();
		 if(recetas != null) {
	 ...
	 public ResponseEntity<?> getRecetaById(@PathVariable Long id){
		 RecetaCocina receta = recetaCocinaService.findById(id);
		 if(receta != null) {
	 ...
	 public ResponseEntity<?> getRecetaById_bis(@RequestBody String id){
		 RecetaCocina receta = recetaCocinaService.findById(new Long(id));
		 if(receta != null) {
			 return new ResponseEntity<>(receta, HttpStatus.OK);
		 }else {
			 return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	...
	public ResponseEntity<?> saveReceta(@RequestBody RecetaCocina receta){
